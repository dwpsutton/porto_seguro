...
class prevalence_vectorizer:
    ...
    def train(self,df):
        #
        self.pr_prev= df[self.target_col].mean()
        prior_obs= 20.
        self.alpha= self.pr_prev*prior_obs
    ...
    def parse(self,vec,field_name):
        if field_name in self.field_list:
            return map(lambda x: self.vectorizer[field_name][x] if x in self.vectorizer[field_name] else self.pr_prev,vec)
        else:
...
def transform_df(indf):
    new_num_cols= ['negative_one_vals','diff_ind_bin']
...
#    indf['ps_car_13_x_ps_reg_03'] = indf['ps_car_13'] * indf['ps_reg_03']
    ...
    ind_binaries= ['ps_ind_06_bin','ps_ind_07_bin','ps_ind_08_bin','ps_ind_09_bin','ps_ind_10_bin','ps_ind_11_bin','ps_ind_12_bin','ps_ind_13_bin','ps_ind_16_bin','ps_ind_17_bin','ps_ind_18_bin']
    indf.loc[:,'diff_ind_bin']= indf.loc[:,ind_binaries].abs().sum(axis=0)
...
#            new_bin_cols.append(c+str('_median_range'))
#            new_bin_cols.append(c+str('_mean_range'))
    return indf, new_num_cols
...
def run_pipe1(train_df,test_df):
    ...
    train_df, new_num= transform_df(train_df)
    ...
    toPrevalate= ['ps_car_06_cat','ps_car_01_cat','ps_car_11','diff_ind_bin']
    num_cols.remove('ps_car_11')
    num_cols.remove('diff_ind_bin')
    ...
    for c in test_df.columns:
        if 'calc' in c:
            ...
            if c in bin_cols:
                ...
                cat_cols.remove(c)
    test_df,_= transform_df(test_df)
